...
int main(int argc, char** argv) {
    ...
    // ofstream avx512_file; /* Enable onle for single-file-logging! */
    // avx512_file.open("./data/512_64bits/results.dat"); /* Enable onle for single-file-logging! */
    ...
    for (int stride_pow = 0; stride_pow <= max_stride; stride_pow++) {
        ...
        // log_multithreaded_results( avx512_file, stride_size, "scalar", scalar );  /* Enable onle for single-file-logging! */
        // log_multithreaded_results( avx512_file, stride_size, "linear", linear );  /* Enable onle for single-file-logging! */
        ...
        log_multithreaded_results_per_file( "./data/512_64bits/results_" + std::to_string(p), stride_pow, { &scalar, &linear, &gather, &seti }, first_run );
        if ( first_run ) {
            first_run = false;
        }
    }
    ...
    // avx512_file.close(); /* Enable onle for single-file-logging! */
    free(array_64);
